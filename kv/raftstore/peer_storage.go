...
func ClearMeta(engines *engine_util.Engines, kvWB, raftWB *engine_util.WriteBatch, regionID uint64, lastIndex uint64) error {
	...
	log.Warnf("file-[%s] clear region [%d] apply meta key {%v}", engines.KvPath, regionID, meta.ApplyStateKey(regionID))
...
func (ps *PeerStorage) Append(entries []eraftpb.Entry, raftWB *engine_util.WriteBatch) error {
	...
	// remove previously appended logs
	nLen := len(entries)
	...
	// update local raft state
	ps.raftState.LastIndex = curLastLogIdx
	ps.raftState.LastTerm = curLastLogTerm
	return nil
}
...
func (ps *PeerStorage) ApplySnapshot(snapshot *eraftpb.Snapshot, kvWB *engine_util.WriteBatch, raftWB *engine_util.WriteBatch) (*ApplySnapResult, error) {
	...
	fmt.Println("===", snapData.Region, ", ", ps.region)

	// check snapshot overlap with current region
